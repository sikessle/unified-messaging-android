...
public class JiraAssignedIssuesCountCheckerTest {
    ...
    private JiraAssignedIssuesCountChecker sut;
    private final ObjectMapper mapper = new ObjectMapper();
    private JiraService mockJiraService;
    private Collection<JiraIssue> expectedIssues;

    @Before
    public void setUp() throws Exception {
        mockJiraService = mock(JiraService.class);

        expectedIssues = new ArrayList<>();
        expectedIssues.add(new JiraIssueBean(ISSUE_KEY, ISSUE_NAME, USER1, ISSUE_HOURS_PER_UPDATE, MetricsService.Category.CRITICAL.toString(), ""));

        Collection<JiraUser> users = new ArrayList<>();
        users.add(new JiraUserBean(USER1, expectedIssues, MetricsService.Category.CRITICAL.toString()));
        ...
        sut = new JiraAssignedIssuesCountChecker(mockJiraService, () -> "");
    }
    ...
    public void testCheck() throws Exception {
        ...
        expectedViolations.add(new MetricViolation("assignedIssuesCountViolated", USER1, pushContent));

        final Collection<MetricViolation> actualViolations = sut.check(getFakeClients());
        verify(mockJiraService).getUsers(eq(PROJECT1), any());
    ...
    private @NotNull ObjectNode createPushContent() {
        ...
        final ArrayNode issues = content.putArray("issues");

        expectedIssues.forEach(issue -> {
            final ObjectNode issueObject = issues.addObject();
            issueObject.put("key", issue.getKey());
            issueObject.put("name", issue.getName());
        });

        content.put("count", issues.size());
